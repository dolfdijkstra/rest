...
public class RESTImport {
    ...
    public void importAll(Set<String> pubs, Set<String> assettypes) throws Exception {
        /*
        ...
        final RestService restService = new DefaultRestService(connectionManager, location);
        long t = System.currentTimeMillis();
        int count = 0;
        boolean doAssets = true;
        AssetXom xom = new AssetXom();
        AssetComparator comp = new AssetComparator();
        try {
                    ...
                    for (final AssetType assetType : restService.getAssetTypes(pub.getName())) {
                        if (assettypes.contains(assetType.getName())) {
                            ...
                            for (final AssetMetaData amd : restService.getAssetMetaData(pub.getName(),
                                    assetType.getName())) {
                                System.out.println(amd.getAssetType() + " " + amd.getId() + " " + amd.getName());
                                if (doAssets) {
                                    try {
                                        // get the actual asset
                                        InputStream stream = restService.getAssetAsStream(amd.getAssetType(),
                                                amd.getId());
                                        Document doc = this.createDocument(stream);

                                        final Asset asset = xom.toObject(doc);
                                        File f = new File(pub.getName() + File.separator + assetType.getName()
                                                + File.separator + asset.getId() + ".xml");
                                        if (f.exists()) {
                                            InputStream stream2 = new FileInputStream(f);
                                            Document doc2 = this.createDocument(stream2);

                                            final Asset asset2 = xom.toObject(doc2);
                                            if (comp.compare(asset, asset2) == -1) {
                                                stream2 = new FileInputStream(f);
                                                // if asset data needs to be
                                                // merged, it needs to be merged
                                                // at the XML DOM level. The
                                                // Asset class does not support
                                                // write function
                                                restService.setAsset(pub.getName(), assetType.getName(), asset.getId(),
                                                        stream2);
                                                stream2.close(); // TODO better
                                                                 // exception
                                                                 // handling.
                                            }
    ...
    private Document createDocument(final InputStream xml) throws DocumentException {
    ...
    public static void main(final String[] a) throws Exception {
        ...
        sample.importAll(pubs, assetTypes);
