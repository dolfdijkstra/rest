...
public class AssetXom implements XOM<Asset> {
    ...
    private final XPath xpathSelector = DocumentHelper.createXPath("/document/asset");
    ...
    Object doData(final Element eav, final FieldType type) throws Exception {
        ...
        switch (type) {
            ...
            case assetreference:
                ...
                final AssetReferenceField arf = new AssetReferenceField(Long.parseLong(eav.attributeValue("value")),
                        eav.attributeValue("type"));
                return arf;
            case binary:
            ...
            case struct: {
                ...
                for (final Iterator<Element> i2 = eav.elementIterator("field"); i2.hasNext();) {
                    final Element field = i2.next();
            ...
            case row: {
                final Map<String, Object> map = new HashMap<String, Object>();
                for (final Iterator<Element> i2 = eav.elementIterator("column"); i2.hasNext();) {
                    final Element field = i2.next();
                    final Element child = (Element) field.elements().get(0);
                    final FieldType type2 = FieldType.parse(child.getName());
                    map.put(field.attributeValue("name"), doData(child, type2));
